...
def main():
    ...
    for pair in cfg.pairs:
        ...
        if not eval_csv.exists():
            print("missing eval:", eval_csv)
            continue
        if not diag_csv.exists():
            print("missing diagnostics:", diag_csv)
            continue
        if not edge_path.exists():
            print("missing edge dataset:", edge_path)
            continue
    ...
    out_path = Path(args.out) if args.out else cfg.analysis_dir / f"{cfg.model_tag}_config_summary.csv"
