...
for R in "${LEVELS[@]}"; do
  ...
  SRC="$HOME/remapgnn/data/MIRA-Datasets/Meshes/UniformlyRefined/CS/sample_NM16_O10_CS-r${R}_TPW_CFR_TPO_A1_A2.nc"
  TGT="$HOME/remapgnn/data/MIRA-Datasets/Meshes/UniformlyRefined/ICOD/sample_NM16_O10_ICOD-r${R}_TPW_CFR_TPO_A1_A2.nc"
...

  {
    ...
    if [[ ! -f "$OV" ]]; then
      ...
      echo "Overlap exists, skipping: $OV"
    fi

    if [[ ! -f "$MAP" ]]; then
      echo "Generating first-order conservative map..."
      GenerateOfflineMap \
        --in_mesh "$SRC" \
        --out_mesh "$TGT" \
        --ov_mesh "$OV" \
